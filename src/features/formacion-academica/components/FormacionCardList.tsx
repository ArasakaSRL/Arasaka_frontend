import { Calendar, GraduationCap, ArrowUpRight, School } from "lucide-react"
...
export default function FormacionCardList({ formaciones }: Props) {
    if (formaciones.length === 0) {
        return (
            <div className="text-center py-20 bg-slate-50/40 rounded-3xl border-2 border-dashed border-slate-200/60 backdrop-blur-sm max-w-4xl mx-auto w-full">
                ...
                <p className="text-slate-600 font-semibold text-base">Tu historial académico está impecable</p>
                <p className="text-slate-400 text-xs mt-1 max-w-xs mx-auto">
                    Aún no has agregado registros. Comienza pulsando el botón superior.
                </p>
            </div>
        )
    }
    ...
    const getNivelBadgeStyles = (nivel: string) => {
        const base = "text-[10px] px-3 py-0.5 rounded-full font-bold tracking-wider uppercase border "
        switch (nivel) {
            case 'Licenciatura': return base + "bg-blue-50/80 text-blue-700 border-blue-100"
            case 'Maestria': return base + "bg-indigo-50/80 text-indigo-700 border-indigo-100"
            case 'Doctorado': return base + "bg-purple-50/80 text-purple-700 border-purple-100"
            case 'Tecnico': return base + "bg-emerald-50/80 text-emerald-700 border-emerald-100"
    ...
    return (
        <div className="max-w-5xl mx-auto w-full space-y-4 antialiased">
            {formaciones.map((item, index) => {
                ...
                const anioInicio = item.fecha_inicio ? item.fecha_inicio.split('-')[0] : '—'
                const anioFin = item.fecha_fin ? item.fecha_fin.split('-')[0] : '—'

                return (
                    <div 
                        key={item.id_formacion || index} 
                        className="group relative bg-white hover:bg-slate-50/30 rounded-2xl border border-slate-100 hover:border-slate-200/70 p-5 transition-all duration-300 ease-out shadow-[0_2px_12px_-4px_rgba(0,0,0,0.02)] hover:shadow-[0_12px_30px_-10px_rgba(30,42,94,0.06)] flex flex-col md:flex-row md:items-center justify-between gap-5 overflow-hidden"
                    >
                            ...
                            <div className="space-y-1 flex-1 min-w-0">
                                ...
                                {item.descripcion && (
                                    <div className="max-h-0 opacity-0 group-hover:max-h-20 group-hover:opacity-100 group-hover:mt-3 transition-all duration-300 ease-in-out overflow-hidden">
                                        <p className="text-xs text-slate-400 bg-slate-50/60 p-3 rounded-xl border border-slate-100/50 italic font-normal max-w-3xl">
                                            {item.descripcion}
                                        </p>
                                    </div>
                                )}
                        ...
                        <div className="flex items-center justify-end md:border-l md:border-slate-100 md:pl-8 shrink-0">
                            <div className="w-9 h-9 rounded-full bg-slate-50 group-hover:bg-[#1e2a5e] flex items-center justify-center text-slate-400 group-hover:text-white transition-all duration-300 transform group-hover:rotate-45 shadow-sm">
