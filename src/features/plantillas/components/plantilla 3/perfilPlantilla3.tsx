...
import type { Usuario } from "@/features/portafolio/types/portafolioType";

type Props = {
  usuario: Usuario;
};

export default function PerfilBubble({ usuario }: Props) {
  const profesion =
    usuario.profesiones?.map((p) => p.nombre).join(" • ") ||
    "Profesional";
